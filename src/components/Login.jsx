...
const Login = ({isUserLogged, setIsUserLogged, handleChange, userEmail, userPassword}) => {
            ...
            <section className="login-box">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="login__data">
                        ...
                        <input value={userEmail} type="text" {...register("email", {required: true, pattern: /^\S+@\S+$/i})}/>
                        {errors.email?.message && <p className="errorMsg">{errors.email?.message}</p>}
                        <label className="label">Hasło</label>
                        <input value={userPassword} type="password" {...register("password", {required: true, max: 15, min: 6})} />
